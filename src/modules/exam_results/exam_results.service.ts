...
import { ExamQuestion, QuestionContent } from '../exam_question/schemas/exam_question.schema';
...
export class ExamResultsService {
  constructor(
    ...
    @InjectModel(ExamQuestion.name) private examQuestionModel: Model<ExamQuestionModule>
  ) {}
...
async getExamComparison(
  ...
  for (const ua of userAnswers) {
    const partIdValue = ua.partId instanceof Types.ObjectId
      ? ua.partId
    ...
    for (let questionIdx = 0; questionIdx < examQuestions.length; questionIdx++) {
      ...
      for (let contentIdx = 0; contentIdx < content.length; contentIdx++) {
        ...
        const comparison: QuestionComparison = {
          ...
          questionNumber: `${questionNumber}.${contentIdx + 1}`, // ✅ Reset mỗi question
          ...
          userAnswerText: userAnswerObj?.selectedAnswer !== undefined && userAnswerObj?.selectedAnswer !== null
            ? (questionContent.answers?.[userAnswerObj.selectedAnswer] ?? 'N/A')
            : 'Chưa chọn đáp án',
          correctAnswerText: questionContent.answers?.[questionContent.correctAnswer] ?? 'N/A',
