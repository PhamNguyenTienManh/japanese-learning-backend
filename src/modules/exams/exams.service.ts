...
export class ExamsService {
  constructor(
    ...
    private examQuestionModel: Model<ExamQuestion>
  ) {}
  ...
  async updateExam(examId: string, data: UpdateExamDto): Promise<any> {
    try {
      if (!Types.ObjectId.isValid(examId)) {
      ...
      if (data.title || data.level) {
        ...
        if (duplicate) {
          throw new BadRequestException(
            `Another exam with title "${
              data.title ?? existingExam.title
            }" already exists for level "${data.level ?? existingExam.level}".`
          );
        }
      }
